namespace Microsoft.Practices.EnterpriseLibrary.Data.Sql
{
    ...
    [DatabaseAssembler(typeof(SqlDatabaseAssembler)), SqlClientPermission(SecurityAction.Demand)]
    public class SqlDatabase : Database
    {
        public SqlDatabase(string connectionString) : base(connectionString, SqlClientFactory.Instance)
        {
        }
        ...
        private XmlReader DoExecuteXmlReader(SqlCommand sqlCommand)
        {
            ...
            try
            {
                ...
                base.instrumentationProvider.FireCommandExecutedEvent(now);
                reader2 = reader;
            }
            catch (Exception exception)
            {
                base.instrumentationProvider.FireCommandFailedEvent(sqlCommand.CommandText, base.ConnectionStringNoCredentials, exception);
                throw;
            }
            return reader2;
        }
