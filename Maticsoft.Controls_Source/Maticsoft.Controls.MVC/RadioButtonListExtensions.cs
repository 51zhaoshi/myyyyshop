namespace Maticsoft.Controls.MVC
{
    ...
    public static class RadioButtonListExtensions
    {
        internal static string CreateRadioButtonItem(SelectListItem info, string name, IDictionary<string, object> htmlAttributes)
        {
            ...
            builder2.MergeAttribute("type", "radio");
        ...
        public static MvcHtmlString RadioButtonList(this HtmlHelper htmlHelper, string name, IEnumerable<SelectListItem> listInfo)
        {
            return htmlHelper.RadioButtonList(name, listInfo, null, 0);
        }

        public static MvcHtmlString RadioButtonList(this HtmlHelper htmlHelper, string name, IEnumerable<SelectListItem> listInfo, object htmlAttributes)
        {
            return htmlHelper.RadioButtonList(name, listInfo, new RouteValueDictionary(htmlAttributes), 0);
        }

        public static MvcHtmlString RadioButtonList(this HtmlHelper htmlHelper, string name, IEnumerable<SelectListItem> listInfo, IDictionary<string, object> htmlAttributes, int number)
        {
            ...
            foreach (SelectListItem item in listInfo)
            {
                ...
                builder2.MergeAttribute("name", name);
                builder2.MergeAttribute("id", str);
                builder.Append(builder2.ToString(TagRenderMode.Normal));
                TagBuilder builder3 = new TagBuilder("label");
                builder3.MergeAttribute("for", str);
        ...
        public static MvcHtmlString RadioButtonList(this HtmlHelper htmlHelper, string name, IEnumerable<SelectListItem> listInfo, IDictionary<string, object> htmlAttributes, Position position = 0, int number = 0)
        {
            if (string.IsNullOrEmpty(name))
            {
                throw new ArgumentException("No Tag Name", "name");
            ...
            switch (position)
            {
                case Position.Horizontal:
                    ...
                    {
                        num++;
                        builder.Append(CreateRadioButtonItem(item, name, htmlAttributes));
                        if ((number == 0) || ((num % number) == 0))
                        {
                            builder.Append("<br />");
                        }
                ...
                case Position.Vertical:
                {
                    ...
                    builder2.MergeAttribute("style", "float: left; light-height: 25px; padding-right: 5px;");
                    ...
                    {
                        num++;
                        builder.Append(CreateRadioButtonItem(item2, name, htmlAttributes));
                        if (num.Equals(num3))
                        {
                            builder.Append(str3);
                            num = 0;
        ...
        public static MvcHtmlString RadioButtonVertical(this HtmlHelper htmlHelper, string name, IEnumerable<SelectListItem> listInfo, IDictionary<string, object> htmlAttributes, int columnNumber = 1)
        {
            if (string.IsNullOrEmpty(name))
            {
            ...
            builder.MergeAttribute("style", "float: left; light-height: 25px; padding-right: 5px;");
