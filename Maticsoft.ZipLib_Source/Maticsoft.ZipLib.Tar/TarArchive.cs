namespace Maticsoft.ZipLib.Tar
{
    ...
    public class TarArchive : IDisposable
    {
        ...
        public event ProgressMessageHandler ProgressMessageEvent
        {
            add
            {
                ProgressMessageHandler handler2;
                ProgressMessageHandler progressMessageEvent = this.ProgressMessageEvent;
                do
                {
                    handler2 = progressMessageEvent;
                    ProgressMessageHandler handler3 = (ProgressMessageHandler) Delegate.Combine(handler2, value);
                    progressMessageEvent = Interlocked.CompareExchange<ProgressMessageHandler>(ref this.ProgressMessageEvent, handler3, handler2);
                }
                while (progressMessageEvent != handler2);
            }
            remove
            {
                ProgressMessageHandler handler2;
                ...
                {
                    handler2 = progressMessageEvent;
                    ProgressMessageHandler handler3 = (ProgressMessageHandler) Delegate.Remove(handler2, value);
                    progressMessageEvent = Interlocked.CompareExchange<ProgressMessageHandler>(ref this.ProgressMessageEvent, handler3, handler2);
                }
                while (progressMessageEvent != handler2);
            }
