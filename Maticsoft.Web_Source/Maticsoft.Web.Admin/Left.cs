namespace Maticsoft.Web.Admin
{
    ...
    public class Left : PageBaseAdmin
    {
        ...
        public void LoadMenu(DataTable dt, int NodeId)
        {
            ...
            if (!flag)
            {
                builder.Append("<div style=\"height: 5px\"></div>");
                foreach (DataRow row in rowArray)
                {
                    ...
                    {
                        builder.AppendFormat("<div class=\"leftothermenu\"><a href=\"{0}\" target=\"mainFrame\">{1}</a></div>", str4, str3);
                    }
                }
            }
            else
        ...
        public string LoadMenu3(DataRow[] dr3)
        {
            ...
            foreach (DataRow row in dr3)
            {
                ...
                if ((permissionid == -1) || base.UserPrincipal.HasPermissionID(permissionid))
                {
                    builder.AppendFormat("<li><a href=\"{0}\" target=\"mainFrame\">{1}</a></li>", str3, str2);
                }
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if ((!this.Page.IsPostBack && (base.Request.Params["id"] != null)) && (base.Request.Params["id"].ToString() != ""))
            {
                ...
                DataSet allTree = tree.GetAllTree();
                this.LoadMenu(allTree.Tables[0], modelByCache.NodeID);
            }
        }
    }
}
