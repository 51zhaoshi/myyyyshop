namespace Maticsoft.Web.Supplier.Products
{
    ...
    public class SelectCategory : PageBaseSupplier
    {
        ...
        private string GetCategoriesList()
        {
            ...
            categorysByParentId.ForEach(delegate (Maticsoft.Model.Shop.Products.CategoryInfo info) {
                data.Add(new JsonObject(new string[] { "CategoryId", "HasChildren", "CategoryName", "Path" }, new object[] { info.CategoryId.ToString(CultureInfo.InvariantCulture), info.HasChildren, info.Name, info.Path }));
            });
            obj2.Put("DATA", data);
            return obj2.ToString();
        }
