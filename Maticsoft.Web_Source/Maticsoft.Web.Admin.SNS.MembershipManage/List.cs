namespace Maticsoft.Web.Admin.SNS.MembershipManage
{
    ...
    {
        protected int Act_BatActive = 0x246;
        protected int Act_BatUnActive = 0x247;
        ...
        protected string GetGravatar(object target)
        {
            ...
            string valueByCache = ConfigSystem.GetValueByCache("SNSGravatarPath");
            if (string.IsNullOrEmpty(valueByCache))
            {
                valueByCache = "/Upload/User/Gravatar/";
            }
            int num = Globals.SafeInt(target.ToString(), 0);
            return (valueByCache + num + ".jpg");
        }
